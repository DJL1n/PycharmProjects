...
def calculate_rectangle_vertices(hole1, hole2, vertex):
    ...
    print((x3, y3), tuple(vertex2), tuple(vertex3), tuple(vertex4))
...
possible=np.arange(378,450,0.5)
...
for i in range(1,25):
    ...
    for t in possible:
        t=int(t*10)
        # print(t)
        # print(v0_addresses[v0_addresses['Time']*10==t].to_dict('records'),t)
        v0_data=v0_addresses[np.isclose(v0_addresses['Time'] * 10, t, atol=1e-6)].to_dict('records')[0]
        
        # print(v0_data)
        # print(22222)
        v1_data=v1_addresses[np.isclose(v1_addresses['Time'] * 10, t, atol=1e-6)].to_dict('records')[0]
        ...
        vi_data=vi_addresses[np.isclose(vi_addresses['Time'] * 10, t, atol=1e-6)].to_dict('records')[0]
        vi_1_data=vi_1_addresses[np.isclose(vi_1_addresses['Time'] * 10, t, atol=1e-6)].to_dict('records')[0]
